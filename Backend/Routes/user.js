...
router.post("/signup",async (req,res)=>{
    ...
    if(response)
    return res.status(200).json({msg:"User or the same username already exists!!, choose another username or try Signing in"});
    ...
    // const encrypted= encryptUserId(id,encryptionKey);
...
router.post("/data",async (req,res)=>{
    ...
    try {
        const response=await User.findOne({username:username})
    if(response)
     return res.json({salt:response.salt});
     
     return res.status(411).json({msg:"Invalid credentials, check your credentials or try signup first /data"});
...
router.post("/auto",initialFetchMiddleware,async (req,res)=>{
    ...
    // const id=req.user_id.id;
    // // const decrypted=decryptUserId(send.user_id,encryptionKey);
    // try {
    //    const response=await User.findOne({_id:id}) 
    //    if(response){

       return res.status(200).json({msg:"Allow auto-login",username:req.response.username,array:req.response.array});
    //    }
    //    else 
    //    return res.status(200).json({msg:"re-login, nice try hacker"});
    // } catch (error) {
    //     res.status(500).json({msg:"Oops! server down, can't fetch user data!!!"});
    // }
...
router.post("/manager",async(req,res)=>{
    ...
    try {
      const response= await User.updateOne({_id:id},{
       ...
       return res.status(500).json({msg:"Oops! serever down, can't upload user data!!!"});
    }
...
router.post("/password",async (req,res)=>{
    ...
    const tk= take.split('').filter(char => (char !== '.'&& char!=='-')).join('');
    const sp="@#!$&";
